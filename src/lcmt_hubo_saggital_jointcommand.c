...
#include <string.h>
#include "lcmt_hubo_saggital_jointcommand.h"

static int __lcmt_hubo_saggital_jointcommand_hash_computed;
static int64_t __lcmt_hubo_saggital_jointcommand_hash;

int64_t __lcmt_hubo_saggital_jointcommand_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __lcmt_hubo_saggital_jointcommand_get_hash)
            return 0;

    const __lcm_hash_ptr cp = { p, (void*)__lcmt_hubo_saggital_jointcommand_get_hash };
    (void) cp;

    int64_t hash = 0x66cb16ef81b1a906LL
...
{
    if (!__lcmt_hubo_saggital_jointcommand_hash_computed) {
        __lcmt_hubo_saggital_jointcommand_hash = __lcmt_hubo_saggital_jointcommand_hash_recursive(NULL);
        __lcmt_hubo_saggital_jointcommand_hash_computed = 1;
    }

    return __lcmt_hubo_saggital_jointcommand_hash;
}
...
{
    int pos = 0, thislen;
    int64_t hash = __lcmt_hubo_saggital_jointcommand_get_hash();

    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    thislen = __lcmt_hubo_saggital_jointcommand_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return 8 + __lcmt_hubo_saggital_jointcommand_encoded_array_size(p, 1);
}
...
int lcmt_hubo_saggital_jointcommand_decode(const void *buf, int offset, int maxlen, lcmt_hubo_saggital_jointcommand *p)
{
    ...
    thislen = __lcmt_hubo_saggital_jointcommand_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return __lcmt_hubo_saggital_jointcommand_decode_array_cleanup(p, 1);
}
...
{
    lcmt_hubo_saggital_jointcommand *q = (lcmt_hubo_saggital_jointcommand*) malloc(sizeof(lcmt_hubo_saggital_jointcommand));
    __lcmt_hubo_saggital_jointcommand_clone_array(p, q, 1);
    return q;
}

void lcmt_hubo_saggital_jointcommand_destroy(lcmt_hubo_saggital_jointcommand *p)
{
    __lcmt_hubo_saggital_jointcommand_decode_array_cleanup(p, 1);
    free(p);
}

int lcmt_hubo_saggital_jointcommand_publish(lcm_t *lc, const char *channel, const lcmt_hubo_saggital_jointcommand *p)
{
      int max_data_size = lcmt_hubo_saggital_jointcommand_encoded_size (p);
      uint8_t *buf = (uint8_t*) malloc (max_data_size);
      if (!buf) return -1;
      int data_size = lcmt_hubo_saggital_jointcommand_encode (buf, 0, max_data_size, p);
      if (data_size < 0) {
          free (buf);
          return data_size;
      }
...
struct _lcmt_hubo_saggital_jointcommand_subscription_t {
    lcmt_hubo_saggital_jointcommand_handler_t user_handler;
    void *userdata;
    lcm_subscription_t *lc_h;
};
static
void lcmt_hubo_saggital_jointcommand_handler_stub (const lcm_recv_buf_t *rbuf,
                            const char *channel, void *userdata)
{
    int status;
    lcmt_hubo_saggital_jointcommand p;
    memset(&p, 0, sizeof(lcmt_hubo_saggital_jointcommand));
    status = lcmt_hubo_saggital_jointcommand_decode (rbuf->data, 0, rbuf->data_size, &p);
    if (status < 0) {
        fprintf (stderr, "error %d decoding lcmt_hubo_saggital_jointcommand!!!\n", status);
        return;
    }

    lcmt_hubo_saggital_jointcommand_subscription_t *h = (lcmt_hubo_saggital_jointcommand_subscription_t*) userdata;
    h->user_handler (rbuf, channel, &p, h->userdata);

    lcmt_hubo_saggital_jointcommand_decode_cleanup (&p);
}

lcmt_hubo_saggital_jointcommand_subscription_t* lcmt_hubo_saggital_jointcommand_subscribe (lcm_t *lcm,
                    const char *channel,
                    lcmt_hubo_saggital_jointcommand_handler_t f, void *userdata)
{
    lcmt_hubo_saggital_jointcommand_subscription_t *n = (lcmt_hubo_saggital_jointcommand_subscription_t*)
                       malloc(sizeof(lcmt_hubo_saggital_jointcommand_subscription_t));
    n->user_handler = f;
    n->userdata = userdata;
    n->lc_h = lcm_subscribe (lcm, channel,
                                 lcmt_hubo_saggital_jointcommand_handler_stub, n);
    if (n->lc_h == NULL) {
        fprintf (stderr,"couldn't reg lcmt_hubo_saggital_jointcommand LCM handler!\n");
        free (n);
        return NULL;
    }
    return n;
...
int lcmt_hubo_saggital_jointcommand_unsubscribe(lcm_t *lcm, lcmt_hubo_saggital_jointcommand_subscription_t* hid)
{
    ...
    if (0 != status) {
        fprintf(stderr,
           "couldn't unsubscribe lcmt_hubo_saggital_jointcommand_handler %p!\n", hid);
        return -1;
    }
    free (hid);
    return 0;
